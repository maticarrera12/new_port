...
export default function WorkSection() {
  ...
  useEffect(() => {
    ...
    const createTextAnimationPath = (yPos: number, amplitude: number) => {
      ...
      ;(line as THREE.Line & { curve: THREE.CatmullRomCurve3 }).curve = curve
      return line as THREE.Line & { curve: THREE.CatmullRomCurve3 }
    }
    ...
    path.forEach((line, i) => {
      ;(line as THREE.Line & { letterElements: HTMLElement[] }).letterElements = Array.from({ length: 15 }, () => {
    ...
    const updateTargetPositions = (scrollProgress = 0) => {
      path.forEach((line, lineIndex) => {
        ;(line as THREE.Line & { letterElements: HTMLElement[]; curve: THREE.CatmullRomCurve3 }).letterElements.forEach((element: HTMLElement, i: number) => {
          const point = (line as THREE.Line & { curve: THREE.CatmullRomCurve3 }).curve.getPoint((i / 14 + scrollProgress * lineSpeedMultipliers[lineIndex]) % 1)
